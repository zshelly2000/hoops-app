...
export async function POST(request: Request) {
  ...
  const { data: gameData, error: gameError } = await supabaseAdmin
    .from('games')
    .insert({ universe_id: universeId, session_id, game_number, team1_score, team2_score })
    .select()
    .single()
  ...
  // Insert game_players
  const gamePlayers = [
    ...team1_players.map((player_id) => ({ universe_id: universeId, game_id: game.id, player_id, team: 1 })),
    ...team2_players.map((player_id) => ({ universe_id: universeId, game_id: game.id, player_id, team: 2 })),
  ]
  ...
  if (playersError) {
    await supabaseAdmin.from('games').delete().eq('universe_id', universeId).eq('id', game.id)
